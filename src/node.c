...
bool cmark_node_set_type(cmark_node * node, cmark_node_type type) {
  if (type == node->type)
    return true;
...
bool cmark_node_get_fenced(cmark_node *node, int *length, int *offset, char *character) {
  if (node == NULL) {
    return NULL;
  }
...
int cmark_node_set_fenced(cmark_node * node, bool fenced,
...
bool cmark_node_set_syntax_extension(cmark_node *node, cmark_syntax_extension *extension) {
  if (node == NULL) {
    return false;
  }
...
bool cmark_node_set_n_table_columns(cmark_node *node, int n_columns) {
  if (node == NULL) {
    return false;
  }
...
bool cmark_node_is_table_header(cmark_node *node) {
  if (node == NULL) {
    return false;
  }
...
bool cmark_node_set_is_table_header(cmark_node *node, bool is_table_header) {
  if (node == NULL) {
    return false;
  }
